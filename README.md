...
After Ubuntu is written, `piburn` performs a separate byte-for-byte comparison between the card and a freshly verified and decompressed source image. The full-card test and this post-flash verification cover different write operations: passing the first does not guarantee that a later image write cannot fail. Writes request an explicit `fsync`; `piburn` then unmounts any image partitions that macOS discovers, preventing host services from changing the FAT boot partition before verification. Verification reads bypass the macOS cache. If data differs, `piburn` reports the first differing byte, full and per-block SHA-256 values, and whether repeated direct reads are stable, transient, or inconsistent. Any inconsistent read remains an error.
